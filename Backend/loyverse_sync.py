...
def descontar_stock_loyverse(sku, stock_after):
    ...
    try:
        ...
        # ⚡ Para ver el error exacto si vuelve a fallar
    ...
    for evento in eventos:
        ...
        if tipo == "receipts.update":
            ...
            for item in line_items:
                ...
                if sku_variante:
                    # ⚡ FIX VENTA FÍSICA: Buscamos la variante exacta
            ...
            for item_data in lista_items:
                ...
                if variantes:
                    sku_variante = variantes[0].get("sku")
                    # ⚡ FIX ECO: Sacamos el código padre de "AM9411-3" -> "AM9411"
                    sku_padre = sku_variante.split('-')[0] if sku_variante and '-' in sku_variante else sku_variante
                    ...
                    if sku_padre:
                        ...
                        if not pantalon_db:
                            ...
                            db.add(nuevo)
                            print(f"🌟 Nuevo modelo padre sincronizado: {sku_padre}")
                        else:
                            pantalon_db.nombre = nombre
                            pantalon_db.precio = precio
                            print(f"🔄 Modelo actualizado desde Loyverse: {sku_padre}")
...
def crear_articulo_loyverse(nombre, sku, precio, nombre_categoria="General"):
    ...
    try:
        ...
        # 📦 CREAR EL PANTALÓN ASIGNÁNDOLE LA CATEGORÍA
        payload_dict = {
            "item_name": nombre,
            "category_id": cat_id,
            "track_inventory": True, # ⚡ EL INTERRUPTOR QUE SOLUCIONA EL ERROR 400
            "option1_name": "Talla",
            "variants": [
        ...
        print(f"✅ OMNICANAL: Modelo {sku} creado en Loyverse con sus tallas.")
    except Exception as e:
        error_msg = e.read().decode('utf-8') if hasattr(e, 'read') else str(e)
...
def crear_cliente_loyverse(nombre, correo, telefono):
    try:
        ...
        print(f"👤 Sincronización de Cliente: {nombre} guardado en Loyverse.")
    except Exception as e:
...
def eliminar_articulo_loyverse(sku):
    try:
        ...
        print(f"🗑️ OMNICANAL: El modelo {sku} fue destruido de Loyverse.")
    except Exception as e:
        error_msg = e.read().decode('utf-8') if hasattr(e, 'read') else str(e)
